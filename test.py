...
def recommender(product):
    product_index = fashion[fashion['name'] == product].index[0]
    ...
    for product_index, similarity_score in top_10_similar_product:
        product_info = {
            'name': fashion.loc[product_index]['name'],
            'image': fashion.loc[product_index]['image']
        }
...
product_name = st.sidebar.selectbox('Select a product:', fashion['name'])
