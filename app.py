...
fashion = pickle.load(open('data.pkl', 'rb'))
similarity = pickle.load(open('similarity.pkl', 'rb'))

def recommendation(product):
    fashion_index = fashion[fashion['name']==product].index[0]
    similarity_list = list(enumerate(similarity[fashion_index]))
    ...
    for idx, similar_score in top_10_similar_product:
        ...
        product_info = {
            'name':fashion.loc[idx]['name'],
            'image':fashion.loc[idx]['image'],
        }
...
product_name = st.selectbox('Select a product: ', fashion['name'])
